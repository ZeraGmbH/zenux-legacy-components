...
void SCPIInterface::onScpiTransaction(ResourceServer::IClientMultiton *t_clientMultiton, const ProtobufMessage::NetMessage_ScpiCommand &t_pbSCPICommand)
{
    ...
    const cSCPICommand command(QString("%1 %2").
                               arg(QString::fromStdString(t_pbSCPICommand.command()),
                                   QString::fromStdString(t_pbSCPICommand.parameter())));
    ScpiObjectPtr tmpObject = m_scpiInstance.getSCPIObject(command); //check which scpi node is triggered

    if(tmpObject!=nullptr) {
        bool retVal=false;
        /// @todo remove debug code
        QString answer = QString("Command: %2 Params(%3): %4;").arg(command.getCommand()).arg(command.getParamCount()).arg(command.getParamList().join(';'));
...
bool SCPIInterface::scpiAddResource(cSCPICommand t_command, ResourceServer::IClientMultiton *t_clientMultiton, QString &t_answerOut)
{
    ...
    if(t_command.getParamCount()==CommandParams::_paramcount) {
        ...
        if(portConvert) { // check if the provider has a valid TCP/IP port where the resource is located
            ...
            cSCPICommand tmpCommand(QString("%1:%2").
                                    arg(scpiHierarchy.join(':'), newResource->getName()));
